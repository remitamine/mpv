...
static bool render_frame(struct vo *vo)
{
    ...
    if (!in->frame_queued && (!in->vsync_timed || in->paused))
        goto nothing_done;
    ...
    if (in->frame_queued) {
        ...
        in->frame_queued = NULL;
    } else if (!in->vsync_timed || !in->current_frame || !in->hasframe) {
        goto nothing_done;
    }
    ...
    assert(in->current_frame); // nothing can unref and change it while rendering
